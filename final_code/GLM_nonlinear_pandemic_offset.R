...
data <- read.csv("../data/62days_pop.csv", header = TRUE) 

data2 <- read.csv("../data/31days_pop.csv", header = TRUE) 
...
mod.1 <- glm( NumberOfEligibleReferrals62DayStandard ~ offset(logLE)+ HB + CancerType + Pandemic + DateSquared , data = data, family=family1 )
