...
mod.6 <- glm( NumberOfEligibleReferrals31DayStandard ~  offset(logLE) + Date +DateSquared + JustPandemic , data = data, family=family1 )
mod.4 <- glm( NumberOfEligibleReferrals31DayStandard ~  offset(logLE) + Date +Pandemic_cat + JustPandemic , data = data, family=family1 )
mod.5 <- glm( NumberOfEligibleReferrals31DayStandard ~  offset(logLE) + Date +DateSquared +Pandemic_cat+ JustPandemic , data = data, family=family1 )
...
write.csv(results,"results/31_eligible_ALL_ALL_Pop_Time_Pan.csv")
...
png(file="results/31_eligible_ALL_together_plot.png",
    width=600, height=350)
plot(data_plot$Date, data_plot$rates,
     main="Nr of 31 day eligible referrals in Scotland for all cancer types",
...
legend("topleft",
       c("actual","fitted model 1", 'fitted model 2', 'fitted model 3', 'fitted model 4', 'fitted model 5'),
       fill=c("black","blue",'red','orange','green', 'brown'))
dev.off()
